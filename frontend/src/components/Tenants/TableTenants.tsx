...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/tenants/tenantsSlice';
...
const TableSampleTenants = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (tenantsNotify.showNotification) {
      notify(tenantsNotify.typeNotification, tenantsNotify.textNotification);
    }
  }, [tenantsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/tenants/tenants-view/?id=${id}`);
  };
